...
class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        mid = (len1 + len2) // 2
        if (len1 + len2) % 2 == 1:
            return float(self.__find_kth(nums1, nums2, mid + 1))
        else:
            lhs = self.__find_kth(nums1, nums2, mid)
            rhs = self.__find_kth(nums1, nums2, mid + 1)
            return (lhs + rhs) / 2.0

    def __find_kth(self, nums1: List[int], nums2: List[int], k: int) -> int:
        """find kth element in num1 and num2, k is index start from 1."""
        len1, len2 = len(nums1), len(nums2)
        # make sure that nums1 is always the smaller one.
        if len1 > len2:
            return self.__find_kth(nums2, nums1, k)
        ...
        if nums1[i1 - 1] < nums2[i2 - 1]:
            return self.__find_kth(nums1[i1:], nums2[:], k - i1)
